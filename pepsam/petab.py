...
###############################################################################
###############################################################################
class peTab(MutableMapping,
#            sage.structure.sage_object.SageObject
            ):
    '''
    peTab objects behave like dictionaries of values,
...
###############################################################################        
    def __getattribute__(self,name): 
        '''
        for access to the parameters-expressions using '.' we need this 
'''
        if name not in super(peTab,self).__getattribute__('__dict__'):
        #if name not in self.__dict__:
            return super(peTab,self).__getattribute__(name)
        else:
            return self.eval_expr(name)
                   
###############################################################################
#   Next, we need to define dict-like access to 'real' parameters:
#   peTab['item'] === petab.__dict__['item']
#   whole mechanics below depends on that instead of using _dict()
#   as we derive from MutableMapping, all dict-like features
#   are automatically generated from those defined below.
###############################################################################
    def __len__ (self):
        return super(peTab,self).__getattribute__('__dict__').__len__()
        
    def __getitem__ (self, key):
        return super(peTab,self).__getattribute__('__dict__').__getitem__ (key)
        
    def __setitem__ (self, key, value):
        return super(peTab,self).__getattribute__('__dict__').__setitem__ (key,value)

    def __delitem__ (self, key):
        return super(peTab,self).__getattribute__('__dict__').__delitem__ (key)
        
    def __iter__(self):
        return super(peTab,self).__getattribute__('__dict__').__iter__()
...
'''
    ...
    def updatedb(self):
        ...
        existing_record=petabTable.find_one({'_id':self['_id']},projection=['_id'])
